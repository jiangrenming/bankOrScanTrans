...
public class QuanCunThread  extends ComonThread {
    ...
    public void run() {
        ...
        if (mTransCode.equals(TransConstans.TRANS_CODE_QC_FZD)){
            handler.sendMessage(handler.obtainMessage(0x01,"非圈存账户正在划款..."));
        }else if (mTransCode.equals(TransConstans.TRANS_CODE_QC_ZD)){
            handler.sendMessage(handler.obtainMessage(0x01,"圈存账户正在划款..."));
        }
        byte[] msgData = null;
        try {
            msgData = nldPacketHandle.pack(aidlDeviceService, mTransCode, quanCunAccounts, 1);
        } catch (Exception e1) {
            LogUtils.e("组包过程发生异常.... transCode=[" + mTransCode + "]&&dataMap=[" + quanCunAccounts + "]", e1);
            e1.printStackTrace();
        ...
        try{
            ...
            if ("00".equals(resmap.get("respcode"))) {
                ...
                if (calMac.equals(resultMac)) { //MAC验证
                    ...
                    try{
                        UpDateDBUtils.getInstance().deleteReverseRecord(quanCunAccounts.get("batchbillno"),aidlDeviceService);
                        ...
                        //交易成功后保存交易记录
                        UpDateDBUtils.getInstance().saveOrUpdateTransRecord(aidlDeviceService,quanCunAccounts, result);
                        //保存结算
                        UpDateDBUtils.getInstance().saveSettleRecordToDB(quanCunAccounts, result,aidlDeviceService);
                        //发送成功
