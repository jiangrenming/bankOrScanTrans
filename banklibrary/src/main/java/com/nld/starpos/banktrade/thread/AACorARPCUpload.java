...
public class AACorARPCUpload extends ComonThread {
    ...
    public AACorARPCUpload(Context context, Handler handler,AidlDeviceService service,String uri) {
    ...
    public void run() {
        ...
        LogUtils.d("ACC/APRC上送个数："+mRecord.size());
        if (!mRecord.isEmpty()) {
            final int maxTimes = mSendTimes;
            for (final TransRecord mR : mRecord) {
                int times = Integer.parseInt(mR.getReversetimes()) + 1;
                while (times <= maxTimes) {
                    handler.sendMessage(handler.obtainMessage(0x01,"正在第[" + number + "]笔\\n第[" + times+"]次AAC/ARPC上送，请稍等..."));
                    LogUtils.d("正在第[" + number + "]笔\n第[" + times + "]次AAC/ARPC上送");
                    Map<String, String> infoMap = TransactionPackageUtil.getAACorARPCInfo(context,mR);
                    byte[] msgData = null;
                    try {
                        msgData = nldPacketHandle.pack(mService, TransConstans.TRANS_CODE_UPLOAD_AAC_ARPC, infoMap, 1);
                    } catch (Exception e1) {
                        LogUtils.e("AAC/ARPC交易组包过程发生异常:dataMap=[" + infoMap + "]", e1);
                        e1.printStackTrace();
                    ...
                    LogUtils.d("AAC/ARPC交易发送加签的报文=[" + HexUtil.bcd2str(msg) + "]");
                    try{
                        ...
                        LogUtils.d("AAC/ARPC交易接收前置返回的交易数据：[" + resultStr + "]");
                        ...
                        LogUtils.d("AAC/ARPC交易解包后resmap " + StringUtil.map2LineStr(resmap)+"/39域 rescode ="+ resmap.get("respcode"));
                        if ("00".equals(resmap.get("respcode"))) {
                            ...
                            LogUtils.d("AAC/ARPC交易计算mac:" + calMac + " AAC/ARPC消息返回" + resultMac);
                            if (calMac.equals(resultMac)) { //MAC验证
                                number ++; // 计数
                                if ("AC".equals(mR.getStatuscode())) { // 交易拒绝AAC交易，不保存流水
                        ...
                        e.printStackTrace();
                        LogUtils.d("AAC/ARPC交易交易异常：" + e.getMessage());
            ...
            LogUtils.d("AAC/ARPC上送结束：" + number);
