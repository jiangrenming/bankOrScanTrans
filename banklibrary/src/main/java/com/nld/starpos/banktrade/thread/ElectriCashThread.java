...
/**
 * Created by jiangrenming on 2017/12/19.
 * 电子现金(普通消费与快速消费)
 */

public class ElectriCashThread extends ComonThread {

    private AidlDeviceService aidlDeviceService;
    private String url;
    private String mTransCode;
    private Map<String,String> electriCashMaps;
    private NldPacketHandle nldPacketHandle;

    public ElectriCashThread(Context context, Handler handler
                                ,String uri,String transCode
                                ,AidlDeviceService service
                                ,Map<String,String> mElectriCashMaps) {
        super(context, handler);
        this.url = uri;
        this.mTransCode = transCode;
        this.aidlDeviceService = service;
        this.electriCashMaps = mElectriCashMaps;
        try {
    ...
    @Override
    public void run() {
        if (mTransCode.equals(TransConstans.TRANS_CODE_DZXJ_TRANS_PUTONG_CONSUMER)){  // 普通消费
            handler.sendMessage(handler.obtainMessage(0x01,"电子普通支付正在进行中..."));
        }else if (mTransCode.equals(TransConstans.TRANS_CODE_DZXJ_TRANS_QUICK_PAY) ){  //快速支付
            handler.sendMessage(handler.obtainMessage(0x01,"电子快速支付正在进行中..."));
        }
        byte[] msgData = null;
        try {
            msgData = nldPacketHandle.pack(aidlDeviceService, mTransCode, electriCashMaps, 1);
        } catch (Exception e1) {
            LogUtils.e("组包过程发生异常.... transCode=[" + mTransCode + "]&&dataMap=[" + electriCashMaps + "]", e1);
            e1.printStackTrace();
        ...
        try{
            ...
            if ("00".equals(resmap.get("respcode"))) {
                ...
                String calMac = nldPacketHandle.getMac(aidlDeviceService, pack);
                LogUtils.d("计算mac:" + calMac + "  消息返回" + resultMac);
                if (calMac.equals(resultMac)) { //MAC验证
                    Cache.getInstance().setResultMap(resmap);
                    try{
                        UpDateDBUtils.getInstance().deleteReverseRecord(electriCashMaps.get("batchbillno"),aidlDeviceService);
                        ...
                        //交易成功后保存交易记录
                        UpDateDBUtils.getInstance().saveOrUpdateTransRecord(aidlDeviceService,electriCashMaps, result);
                        //保存结算
                        UpDateDBUtils.getInstance().saveSettleRecordToDB(electriCashMaps, result,aidlDeviceService);
                        //发送成功
