...
public class ICKeyQueryThread extends ComonThread {
    ...
    public ICKeyQueryThread(Context context, Handler handler, String transCode
    ...
    @Override
    public void run() {
        if (ICCodeType.equals(TransConstans.TRANS_CODE_IC_KEY_QUERY)){ //IC卡公钥查询
            Map<String, String> icCaQueryInfo = TransactionPackageUtil.getICCaDownInfo(context,"",ICCodeType);
            packAndRequest(ICCodeType,icCaQueryInfo);
        }

    }

    public void packAndRequest(String transType,Map<String,String> maps) {
        handler.sendMessage(handler.obtainMessage(0x01,"IC卡公钥查询中...."));
        ...
        if (pack !=null){
            try{
                ...
                LogUtils.i("IC卡公钥查询接收前置返回的交易数据：["+result+"]");
                String processRequire = result.substring(19, 20);
                LogUtils.e("IC卡公钥查询报文头处理要求："+processRequire);
    ...
    private byte[] getPack( Map<String, String> dataMap,String transCode){
        ...
        System.out.println("IC卡公钥查询请求发送的报文=["+HexUtil.bcd2str(msg)+"]");
