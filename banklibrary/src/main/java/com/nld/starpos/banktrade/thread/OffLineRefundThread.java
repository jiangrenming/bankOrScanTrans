...
public class OffLineRefundThread extends ComonThread {
    private String url;
    ...
    public OffLineRefundThread(Context context, Handler handler
    ...
    @Override
    public void run() {
        handler.sendMessage(handler.obtainMessage(0x01,"脱机退货交易正在请求中..."));
        byte[] msgData = null;
        try {
            msgData = nldPacketHandle.pack(aidlDeviceService, transCode, offlines, 1);
        } catch (Exception e1) {
            LogUtils.e("脱机退款组包过程发生异常.... transCode=[" + transCode + "]&&dataMap=[" + offlines + "]", e1);
            e1.printStackTrace();
        ...
        LogUtils.d("脱机退款发送加签的报文=[" + HexUtil.bcd2str(msg) + "]");
        try{
            ...
            LogUtils.d("脱机退款接收前置返回的交易数据：[" + resultStr + "]");
            byte[] pack = nldPacketHandle.subMessLen(HexUtil.hexStringToByte(resultStr));
            LogUtils.d("脱机退款返回的报文=[" + HexUtil.bcd2str(pack) + "]");
            Map<String, String> resmap = nldPacketHandle.unPack(pack, "002311", 1);//解包
            LogUtils.d("脱机退款解包后resmap " + StringUtil.map2LineStr(resmap)+"/39域 rescode ="+ resmap.get("respcode"));
            if ("00".equals(resmap.get("respcode"))) {
                ...
                if (calMac.equals(resultMac)) { //MAC验证
                    ...
                    try{
                        UpDateDBUtils.getInstance().deleteReverseRecord(offlines.get("batchbillno"),aidlDeviceService);
                        ...
                        //交易成功后保存交易记录
                        UpDateDBUtils.getInstance().saveOrUpdateTransRecord(aidlDeviceService,offlines, result);
                        //保存结算
                        UpDateDBUtils.getInstance().saveSettleRecordToDB(offlines, result,aidlDeviceService);
                        //发送成功
            ...
            e.printStackTrace();
            LogUtils.d("脱机退款交易异常：" + e.getMessage());
            //交易超时需要冲正。
