...
public class ICKeyDownloadThread extends ComonThread {
    ...
    public ICKeyDownloadThread(Context context, Handler handler, String transCode
    ...
    @Override
    public void run() {
        if (ICCodeType.equals(TransConstans.TRANS_CODE_IC_KEY_DOWN)){  //IC卡公钥下载
            Map<String, String> icCaDownInfo = TransactionPackageUtil.getICCaDownInfo(context, mOpearcode,ICCodeType);
            packAndRequest(ICCodeType,icCaDownInfo);
        }
    }

    public void packAndRequest(String transType,Map<String,String> maps) {
        handler.sendMessage(handler.obtainMessage(0x01,"第"+catTime+"条IC卡公钥下载中"));
        ...
        if (pack !=null){
            try{
                ...
                LogUtils.i("IC卡公钥下载接收前置返回的交易数据：["+result+"]");
                String processRequire = result.substring(19, 20);
                LogUtils.e("IC卡公钥下载报文头处理要求："+processRequire);
