...
public class BankConsumeThread extends ComonThread {


    private Map<String,String> mConsume;
    private NldPacketHandle nldPacketHandle;
    private AidlDeviceService deviceService;
    private String mTransCode;
    private String uri;

    public BankConsumeThread(Context context, Handler handler) {
        super(context, handler);
    }

    public BankConsumeThread(Context context, Handler handler, Map<String,String> consume,
                             AidlDeviceService deviceService,String transCode,String uri) {
        super(context, handler);
    ...
    @Override
    public void run() {
        if (TransConstans.TRANS_CODE_CONSUME.equals(mTransCode)){
            handler.sendMessage(handler.obtainMessage(0x01,"消费正在请求中..."));
        } else if (TransConstans.TRANS_CODE_QUERY_BALANCE.equals(mTransCode)){
            handler.sendMessage(handler.obtainMessage(0x01,"余额查询正在请求中..."));
        }else if (TransConstans.TRANS_CODE_CONSUME_CX.equals(mTransCode)){
            handler.sendMessage(handler.obtainMessage(0x01,"消费撤销正在请求中..."));
        }

        byte[] msgData = null;
        try {
            msgData = nldPacketHandle.pack(deviceService, mTransCode, mConsume, 1);
        } catch (Exception e1) {
            LogUtils.e("组包过程发生异常.... transCode=[" + mTransCode + "]&&dataMap=[" + mConsume + "]", e1);
            e1.printStackTrace();
        ...
        try{
            ...
            if ("00".equals(resmap.get("respcode"))) {
                ...
                String calMac = nldPacketHandle.getMac(deviceService, pack);
                LogUtils.d("计算mac:" + calMac + "  消息返回" + resultMac);
                if (calMac.equals(resultMac)) { //MAC验证
                    Cache.getInstance().setResultMap(resmap);
                    try{
                        UpDateDBUtils.getInstance().deleteReverseRecord(mConsume.get("batchbillno"),deviceService);
                        ...
                        //交易成功后保存交易记录
                        UpDateDBUtils.getInstance().saveOrUpdateTransRecord(deviceService,mConsume, result);
                        //保存结算
                        UpDateDBUtils.getInstance().saveSettleRecordToDB(mConsume, result,deviceService);
                        //发送成功
