package com.nld.cloudpos.payment.view.gridpwdview;
...
import com.nld.cloudpos.payment.view.gridpwdview.imebugfiexed.ImeDelBugFixedEditText;
...
public class GridPasswordView extends LinearLayout implements PasswordView {
    ...
    private void inflaterViews(Context context) {
        ...
        //add by lzc 第一个输入框点击无效
        inputView.setOnClickListener(this.onClickListener);
        viewArr[0] = inputView;
    ...
    /**
     *  add by lzc 设置每个输入框的点击事件
     * @param onClick
     */
    public void setHoleViewClick(OnClickListener onClick){
        if(null!=viewArr && viewArr.length>0){
            for(int i=0;i<viewArr.length;i++){
                viewArr[i].setOnClickListener(onClick);
            }
        }
    }
...
//        inputView.setFocusable(true);
    ...
    public void setPasswordVisibility(boolean visible) {
        for (TextView textView : viewArr) {
            ...
            if (textView instanceof MEditText) {
                MEditText et = (MEditText) textView;
