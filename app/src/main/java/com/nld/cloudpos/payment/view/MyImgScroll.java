...
public class MyImgScroll extends ViewPager {
	...
	List<? extends View> mListViews; // 图片组
	int mScrollTime = 0;
	Timer timer;
	int oldIndex = 0;
	int curIndex = 0;
	...
	public void start(Activity mainActivity, List<? extends View> imgList,
                      int scrollTime, LinearLayout ovalLayout, int ovalLayoutId,
                      int ovalLayoutItemId, int focusedId, int normalId, boolean isAdPlay) {
		...
		this.setAdapter(pagerAdapter);// 设置适配器
		if (scrollTime != 0 && imgList.size() > 1) {
			// 设置滑动动画时间 ,如果用默认动画时间可不用 ,反射技术实现

			if (isAdPlay) { // 如果为图片播放，则启动定时器
				startTimer();
	...
	public class MyPagerAdapter extends PagerAdapter {
		...
		private List<? extends View> views = null;
		 ...
		 * @date:2015-7-23 上午12:40:01
		 */
		public void setViews(List<? extends View> data) {
			this.views = data;
		}
